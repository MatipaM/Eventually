...
DocumentSnapshot? snapshot;
  ...
  Contact createContactFromUser()
  {
    ...
    Future<void> getAllAppliedFromFireStore() async{
  bookings = [];
  QuerySnapshot snapshots = await FirebaseFirestore.instance.collection('users/$id/jobs').get(); //initally getDocuments()
...
List<Booking> getAppliedJobs()
{
  List<Booking> appliedJobs = [];

  for (var booking in bookings!) { //this line needs to be updated
    if(booking.dates!.last.compareTo(DateTime.now()) > 0){
      appliedJobs.add(booking);
    } 
   }
